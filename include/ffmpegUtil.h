...
class PacketGrabber {
  ...
  int v_width, v_height;//视频分辨率
  int video_frame_count;//视频总帧数
  int frame_rate;//视频帧率

  int audio_samplerate;  // 采样率
 ...
 public:
  ...
  PacketGrabber(const string& uri) : inputUrl(uri) {
    ...
    if (formatCtx->streams[i]->codec->codec_type == AVMEDIA_TYPE_VIDEO && videoIndex == -1) {
        ...
        {
            frame_rate = in_stream->avg_frame_rate.num / in_stream->avg_frame_rate.den;//每秒多少帧 
        }

        video_frame_count = in_stream->nb_frames; //视频总帧数
        cout << "————————begin——————————" << endl;
        time = (formatCtx->duration) / 1000000;
        cout << "文件播放时长（单位：秒）：" << time << endl;
        av_rate = (formatCtx->bit_rate) / 1000;
        cout << "文件总码率（平均码率）：" << av_rate << endl;

        cout << "视频的宽：" << v_width << endl;
        cout << "视频的高：" << v_height << endl;
        cout << "视频总帧数：" << video_frame_count << endl;
        cout << "视频帧率：" << frame_rate << endl;
    }

      if (formatCtx->streams[i]->codec->codec_type == AVMEDIA_TYPE_AUDIO && audioIndex == -1) {
        audioIndex = i;
        cout << "audio stream index = : [" << i << "]" << endl;
        audio_samplerate = in_stream->codec->sample_rate; // 采样率
        audio_channels = in_stream->codec->channels;  //声道数

        cout << "采样率：" << audio_samplerate << endl;
        cout << "声道数：" << audio_channels << endl;
        cout << "—————————over———————————" << endl;
