...
class MediaProcessor {
  ...
  void nextFrameKeeper() {
    ...
    while (!streamFinished && started) {
      ...
      //cout << "+++++++++" << endl;
      //cout << "+++++++++++  PrepareNextData, index="<< streamIndex <<", prepareintervalTime=" << (diff.count() * 1000) << "ms" <<endl;
      prepareNextData();
    }
...
class AudioProcessor : public MediaProcessor {
 ...
 //       cout << "时间基" << streamTimeBase << endl;
        
      //  auto current_times = frame->pts * av_q2d(streamTimeBase);
     //   cout << "音频时间：" << current_times << endl;
        streamIndex = i;
...
class VideoProcessor : public MediaProcessor {
 ...
 public:
  ...
  VideoProcessor(AVFormatContext* formatCtx) {
    ...
    sws_ctx = sws_getContext(w, h, codecCtx->pix_fmt, w, h, AV_PIX_FMT_RGB24, SWS_BILINEAR,
                             NULL, NULL, NULL);
